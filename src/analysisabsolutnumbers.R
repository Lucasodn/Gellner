...
cor(dat.cor$Gesamt64, dat.cor$countypop1864)
...
###########################################################################
#        ABSOLUT NUMBERS MAIN MODEL Industry worker 1864
###########################################################################
...
modelscitysize$basic <- lm(data = dat_shareindu, Gesamt64 ~ countypop1864)

modelscitysize$with_ind <- lm(data = dat_shareindu, Gesamt64 ~ countypop1864 +
                                occ1864_ind)


modelscitysize$with_prot <- lm(data = dat_shareindu, Gesamt64 ~ countypop1864 +
                                 rel1864_pro)

models$with_edu <- lm(data = dat_shareindu, Gesamt64 ~ countypop1864 +
                        students1864)

modelscitysize$full <- lm(data = dat_shareindu, Gesamt64 ~ countypop1864 +
                            occ1864_ind +
                            rel1864_pro + 
                            students1864)
...
stargazer(modelscitysize,
          out = "../publish/full_modelindustry1864absolut.tex",
          title = "Linear Model Results County Population and Industry Worker 1864",
          covariate.labels = modelnames_citysize,
          dep.var.labels = "Number of Turner", font.size = "tiny")


stargazer(modelscitysize,type = "text",
          out = "../misc/modelindustry1864absolut.txt",
          title = "Linear Model Results County Population and Industry Worker 1864",
          covariate.labels = modelnames_citysize,
...
###########################################################################
#           ABSOLUT NUMBERS MAIN MODEL Enrollment Rate 1864
###########################################################################

models_edu <- list()


models_edu$basic <- lm(data = dat_share, Gesamt64 ~ students1864)

models_edu$with_prot <- lm(data = dat_share, Gesamt64 ~ students1864 +
                             rel1864_pro)

models_edu$with_full <- lm(data = dat_share, Gesamt64 ~ students1864 +
                             rel1864_pro + 
                             countypop1864)
...
plot(models_edu$with_full, 1)
plot(models_edu$with_full, 3)
...
stargazer(models_edu,
          out = "../publish/full_modeleducation1864absolut.tex",
          title = "Linear Model Results Education 1864",
          covariate.labels = modelnames_edu,
          dep.var.labels = "Number of Turner", font.size = "tiny")


stargazer(models_edu,type = "text",
          out = "../misc/modeleducation1864absolut.txt",
          title = "Linear Model results Education 1864",
          covariate.labels = modelnames_edu,
          dep.var.labels = "Number of Turner")
...
###########################################################################
#            ABSOLUT NUMBERS MAIN MODEL Steam Engines 1849 
###########################################################################
...
###########################################################################
#                 MAIN MODEL Farmer Absolut Numbers 1849
###########################################################################

models_farmer <- list()


models_farmer$basic <- lm(data = dat_share, Gesamt64 ~ farmer1849)

models_farmer$with_prot <- lm(data = dat_share, Gesamt64 ~ farmer1849 +
                                rel1864_pro)

models_farmer$with_edu <- lm(data = dat_share, Gesamt64 ~ farmer1849 +
                               students1864)

models_farmer$with_pop <- lm(data = dat_share, Gesamt64 ~ farmer1849 +
                               countypop1849)

models_farmer$full <- lm(data = dat_share, Gesamt64 ~ farmer1849 +
                           rel1864_pro +
                           students1864 +
                           countypop1849)
###########################################################################
# MODEL NAMES
modelnames_farmer<- c("Farmer 1848",
                      "Protestants 1864",
                      "Students 1864",
                      "Countypopulation 1848")


###########################################################################
# EXTRACT MODEL FOR PUBLISHING
stargazer(models_farmer,
          out = "../publish/full_modelfarmerupdateabsolut.tex",
          title = "Linear Model Results Farmer",
...
stargazer(models_farmer,
          type = "text",
          out = "../misc/modelfarmerupdateabsolut.txt",
          title = "Linear Model Results Farmer",
          covariate.labels = modelnames_farmer,
          dep.var.labels = "Number of Turner")
...
###########################################################################
#                Absolut Numbers  MAIN MODEL Factory Worker 1849
###########################################################################

models_factory <- list()


models_factory$basic <- lm(data = dat_share, Gesamt64 ~ all_factoryworker1849)

models_factory$with_prot <- lm(data = dat_share, Gesamt64 ~ all_factoryworker1849 +
                                 rel1864_pro)

models_factory$with_edu <- lm(data = dat_share, Gesamt64 ~ all_factoryworker1849 +
                                students1864)

models_factory$with_craft <- lm(data = dat_share, Gesamt64 ~ all_factoryworker1849 +
                                  crafters)
models_factory$with_craft <- lm(data = dat_share, Gesamt64 ~ all_factoryworker1849 +
                                  countypop1864)

models_factory$full <- lm(data = dat_share, Gesamt64 ~ all_factoryworker1849 +
                            rel1864_pro +
                            students1864 +
                            crafters + 
                            countypop1864)
...
###########################################################################
# MODEL NAMES
modelnames_factory<- c("Factory worker 1849",
                       "Protestants 1864",
                       "Students 1864",
                       "Craftsmen 1849",
                       "Countypopulation 1864")
...
stargazer(models_factory,
          out = "../publish/full_modelfactoryupdateabsolut.tex",
...
stargazer(models_factory,
          type = "text",
          out = "../misc/modelfactoryupdateabsolut.txt",
          title = "Linear Model Results Factory",
          covariate.labels = modelnames_factory,
          dep.var.labels = "Number of Turner")
############################################################################
